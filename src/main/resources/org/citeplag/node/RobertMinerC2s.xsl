...
====== Pass 9 (rewrite type, definitionURL, non-strict attributes ) =======
    ...
    <!-- remove fn -->
    <xsl:template match="fn" mode="pass1">
        <xsl:apply-templates mode="#current"/>
    </xsl:template>
            ...
            <xsl:element name="apply">
                <xsl:choose>
                    <xsl:when test="bvar[degree]">  <!-- nth diff-->
                        <xsl:element name="csymbol"><xsl:attribute name="cd">calculus1</xsl:attribute>nthdiff
                        </xsl:element>
                        <xsl:apply-templates select="bvar/degree/node()" mode="#current"/><!-- n -->
                        <xsl:element name="bind">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                            </xsl:element>
                    ...
                    <xsl:otherwise> <!-- plain diff -->
                        <xsl:element name="csymbol"><xsl:attribute name="cd">calculus1</xsl:attribute>diff
                        </xsl:element>
                        <xsl:element name="bind">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                            </xsl:element>
    ...
    <xsl:template match="apply[partialdiff][bvar]" mode="pass2">
        <xsl:element name="apply">
            <xsl:element name="apply">
                <xsl:element name="csymbol"><xsl:attribute name="cd">calculus1</xsl:attribute>partialdiffdegree
                </xsl:element>
                <xsl:element name="apply"><!-- list of degrees -->
                    <xsl:element name="csymbol"><xsl:attribute name="cd">list1</xsl:attribute>list
                    </xsl:element>
                    <xsl:for-each select="bvar"><!-- n1 - nk, default to 1 if not specified -->
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:element name="cn">1</xsl:element>
                            </xsl:otherwise>
                        </xsl:choose>
                    </xsl:for-each>
                </xsl:element>
                <xsl:choose><!-- total degree -->
                    ...
                    <xsl:otherwise><!-- formal sum of individual degrees -->
                        <xsl:element name="apply">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">arith1</xsl:attribute>plus
                            </xsl:element>
                            <xsl:for-each select="bvar"><!-- n1 - nk, default to 1 if not specified -->
                                <xsl:choose>
                                    <xsl:when test="degree">
                ...
                <xsl:element name="bind"><!-- lambda function to be differentiated -->
                    ...
                    <!-- expr in x -->
                    <xsl:apply-templates select="(bvar|degree)[position()=last()]/following-sibling::*"
                                         mode="#current"/>
                </xsl:element>
            </xsl:element>
    ...
    <xsl:template match="apply[int][bvar]" mode="pass2">
        <xsl:choose>
            <xsl:when test="lowlimit and uplimit"><!-- apply int bvar lowlimit uplimit expr -->
                <xsl:element name="apply">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">calculus1</xsl:attribute>defint
                    </xsl:element>
                    <xsl:element name="apply">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">interval1</xsl:attribute>oriented_interval
                        </xsl:element>
            ...
            <xsl:when test="condition"><!-- apply int bvar condition expr -->
                <xsl:element name="apply">
                    ...
                    <xsl:element name="apply"><!-- set constructed by condition-->
                        <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>suchthat
                        </xsl:element>
                        ...
                        <xsl:element name="bind">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="count(condition)>1">
                                    <xsl:element name="apply">
                                        <xsl:element name="csymbol"><xsl:attribute name="cd">logic1</xsl:attribute>and
                                        </xsl:element>
                                        <xsl:apply-templates select="condition/*" mode="#current"/>
                                    </xsl:element>
                                </xsl:when>
            ...
            <xsl:otherwise><!-- apply int expr -->
                <xsl:element name="apply">
                    <xsl:element name="apply">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">calculus1</xsl:attribute>int
                        </xsl:element>
                        <xsl:element name="bind"><!-- lambda function to be integrated -->
                            <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                            </xsl:element>
                            ...
                            <xsl:apply-templates
                                    select="(bvar|uplimit|lowlimit)[position()=last()]/following-sibling::*"
                                    mode="#current"/><!-- expr in x -->
                        </xsl:element>
                    </xsl:element>
                    <xsl:apply-templates select="bvar/node()" mode="#current"/><!-- x -->
    ...
    <xsl:template match="apply[limit]" mode="pass2">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">limit1</xsl:attribute>limit
            </xsl:element>
            ...
            <xsl:choose>
                <xsl:when test="condition/apply/tendsto/@type='above'">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">limit1</xsl:attribute>above
                    </xsl:element>
                </xsl:when>
                <xsl:when test="condition/apply/tendsto/@type='below'">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">limit1</xsl:attribute>below
                    </xsl:element>
                </xsl:when>
                <xsl:when test="condition/apply/tendsto/@type='both_sides'">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">limit1</xsl:attribute>both_sides
                    </xsl:element>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:element name="csymbol"><xsl:attribute name="cd">limit1</xsl:attribute>null
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
            <!-- lambda expr -->
            <xsl:element name="bind">
                ...
                <!-- expr in bvars -->
                <xsl:apply-templates select="(condition|bvar|lowlimit)[position()=last()]/following-sibling::*"
                                     mode="#current"/>
            </xsl:element>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="apply[sum|product][bvar][lowlimit][uplimit]" mode="pass2">
        <xsl:element name="apply">
            <xsl:choose><!-- head term -->
                <xsl:when test="sum">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">arith1</xsl:attribute>sum
                    </xsl:element>
                </xsl:when>
                <xsl:when test="product">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">arith1</xsl:attribute>product
                    </xsl:element>
                </xsl:when>
            </xsl:choose>
            <xsl:element name="apply"><!-- integer interval -->
                <xsl:element name="csymbol"><xsl:attribute name="cd">interval1</xsl:attribute>integer_interval
                </xsl:element>
                <xsl:apply-templates select="lowlimit/*" mode="#current"/>
            ...
            <xsl:element name="bind"><!-- lambda expr -->
                ...
                <xsl:apply-templates select="bvar" mode="#current"/>
                <xsl:apply-templates select="(bvar|lowlimit|uplimit)[position()=last()]/following-sibling::*"
                                     mode="#current"/>
            </xsl:element>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="apply[root]" mode="pass2">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">arith1</xsl:attribute>root
            </xsl:element>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:element name="cn"><xsl:attribute name="type">integer</xsl:attribute>2
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:element>
    ...
    <xsl:template match="apply[log]" mode="pass2">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">transc1</xsl:attribute>log
            </xsl:element>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:element name="cn"><xsl:attribute name="type">integer</xsl:attribute>10
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template match="apply[moment]" mode="pass2">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">s_dist1</xsl:attribute>moment
            </xsl:element>
            <xsl:choose><!-- degree -->
                <xsl:when test="degree">
                    <xsl:apply-templates select="degree/*" mode="#current"/>
                ...
                <xsl:otherwise>
                    <xsl:element name="cn"><xsl:attribute name="type">integer</xsl:attribute>1
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
            <xsl:choose><!-- point -->
                ...
                <xsl:otherwise>
                    <xsl:element name="cn"><xsl:attribute name="type">integer</xsl:attribute>0
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:template match="apply[bvar][lowlimit][uplimit]" mode="pass3">
        <xsl:element name="apply">
            ...
            <xsl:element name="domainofapplication">
                <xsl:element name="apply">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">interval1</xsl:attribute>interval
                    </xsl:element>
                    <xsl:apply-templates select="lowlimit/*" mode="#current"/>
                    <xsl:apply-templates select="uplimit/*" mode="#current"/>
                </xsl:element>
    ...
    <xsl:template match="*[bvar][condition]" mode="pass3">
        <xsl:copy>
            <xsl:apply-templates select="bvar[position()=1]/preceding-sibling::node()"
                                 mode="#current"/><!-- head term -->
            <xsl:apply-templates select="bvar" mode="#current"/>
            <xsl:element name="domainofapplication">
                <xsl:element name="apply">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>suchthat
    ...
    <xsl:template match="apply[domainofapplication]" mode="pass3">
        <xsl:element name="apply">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:element name="domainofapplication">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>intersection
                        </xsl:element>
                        <xsl:apply-templates select="domainofapplication/*" mode="#current"/>
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
            <xsl:apply-templates select="domainofapplication[position()=last()]/following-sibling::node()"
                                 mode="#current"/>
        </xsl:element>
    </xsl:template>
        ...
        <xsl:choose>
                            ...
                            <xsl:element name="csymbol">
                                <xsl:choose>
                                    <xsl:when test="name()='set'">
                                        <xsl:attribute name="cd">set1</xsl:attribute>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:attribute name="cd">list1</xsl:attribute>
                                    </xsl:otherwise>
                                </xsl:choose>
                                map
                            </xsl:element>
            ...
            <xsl:otherwise><!-- elements are enumerated -->
                <xsl:element name="apply">
                    <xsl:choose>
                        <xsl:when test="name()='set'">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>set
                            </xsl:element>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:element name="csymbol"><xsl:attribute name="cd">list1</xsl:attribute>list
    ...
    <xsl:template match="lambda[bvar]" mode="pass4">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:element name="apply">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>restriction
                    </xsl:element>
                    <xsl:element name="bind">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                        </xsl:element>
    ...
    <xsl:template match="piecewise" mode="pass4">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">piece1</xsl:attribute>piecewise
            </xsl:element>
            <xsl:apply-templates mode="#current"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="piece" mode="pass4">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">piece1</xsl:attribute>piece
            </xsl:element>
            <xsl:apply-templates mode="#current"/>
        </xsl:element>
    </xsl:template>
    ...
    <xsl:template match="otherwise" mode="pass4">
        <xsl:element name="apply">
            <xsl:element name="csymbol"><xsl:attribute name="cd">piece1</xsl:attribute>otherwise
            </xsl:element>
            <xsl:apply-templates mode="#current"/>
        </xsl:element>
    </xsl:template>
        ...
        <xsl:choose>
            ...
            <xsl:when test="bvar and domainofapplication and count(*) >= 4">
                <xsl:element name="apply">
                    <xsl:element name="csymbol">
                        <xsl:attribute name="cd">
                            <xsl:value-of select="$symbols//element/op[string()=$opname]/../cd"/>
                        </xsl:attribute>
                        <xsl:value-of select="*[position()=1]/name()"/>
                    </xsl:element>
                    <xsl:variable name="bvar" select="bvar/*"/>
                    <xsl:variable name="func"
                                  select="(bvar|domainofapplication)[position()=last()]/following-sibling::*"/>
                    <xsl:choose>
                        <!-- the child of the d-o-a is already a strict set contructed earlier -->
                        <xsl:when test="domainofapplication/apply/csymbol[string()='suchthat'] and
			  ...
			  dsi:deep-equal($bvar, $func)">
                        ...
                        <xsl:otherwise>
                            <xsl:element name="apply">
                                <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>map
                                </xsl:element>
                                <xsl:element name="bind">
                                    <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                                    </xsl:element>
                ...
                <xsl:element name="bind">
                    <xsl:choose>
                        <xsl:when test="exists">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">quant1</xsl:attribute>exists
                            </xsl:element>
                        </xsl:when>
                        <xsl:when test="forall">
                            <xsl:element name="csymbol"><xsl:attribute name="cd">quant1</xsl:attribute>forall
                            </xsl:element>
                        </xsl:when>
                    </xsl:choose>
                    <xsl:apply-templates select="./bvar" mode="#current"/>
                    <xsl:choose>
                        <xsl:when test="domainofapplication"><!-- doa is present -->
                            <xsl:element name="apply">
                                <xsl:element name="csymbol">
                                    <xsl:attribute name="cd">logic1</xsl:attribute>
                                    <xsl:choose>
                                ...
                                <xsl:element name="apply">
                                    <xsl:element name="csymbol"><xsl:attribute name="cd">set1</xsl:attribute>in
                                    </xsl:element>
                                    <xsl:apply-templates select="./bvar/*" mode="#current"/>
                                    <xsl:apply-templates select="./domainofapplication/*" mode="#current"/>
                                </xsl:element>
    ...
    <xsl:template match="apply[sum|product][domainofapplication]" mode="pass5">
        <xsl:element name="apply">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:apply-templates
                            select="(sum|product|domainofapplication)[position()=last()]/following-sibling::*"
    ...
    <xsl:template match="apply[domainofapplication]" mode="pass6">
        <xsl:choose>
            ...
            <xsl:when test="dsi:is-nary-reln(.) or dsi:is-nary-set-reln(.)">
                ...
                <xsl:element name="apply">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">fns2</xsl:attribute>predicate_on_list
                    </xsl:element>
                    <!-- head term, TBD: choose CD based on whether type=set|multiset -->
                    <xsl:apply-templates select="node()[position()=1]" mode="#current"/>
                    <xsl:element name="apply">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">list1</xsl:attribute>map
                        </xsl:element>
                        <xsl:element name="bind"><!-- lambda function -->
                            <xsl:element name="csymbol"><xsl:attribute name="cd">fns1</xsl:attribute>lambda
                            </xsl:element>
            ...
            <xsl:when test="dsi:is-nary-set(.)">
                <xsl:element name="apply">
                    <!-- Should we special case when we did just create a strict list??? -->
                    <xsl:element name="csymbol"><xsl:attribute name="cd">fns2</xsl:attribute>apply_to_list
                    </xsl:element>
                    <!-- head term, TBD: choose CD based on whether type=set|multiset -->
                    <xsl:apply-templates select="node()[position()=1]" mode="#current"/>
                    <xsl:element name="apply">
    ...
    <xsl:template match="cn[sep]" mode="pass7">
        <xsl:element name="apply">
            <xsl:element name="csymbol">
                <xsl:choose>
                    <xsl:when test="@type='rational'"><xsl:attribute name="cd">nums1</xsl:attribute>rational
                    </xsl:when>
                    <xsl:when test="@type='e-notation'"><xsl:attribute name="cd">nums1</xsl:attribute>bigfloat
                    </xsl:when>
                    <xsl:when test="@type='complex-cartesian'">
                        <xsl:attribute name="cd">complex1</xsl:attribute>
                        complex_cartesian
                    </xsl:when>
                ...
                <xsl:element name="semantics">
                    <xsl:choose>
                        <xsl:when test="name()='ci'">
                            <xsl:element name="ci">
                                <xsl:value-of select="dsi:name-from-mmlp(.)"/>
                            </xsl:element>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:element name="csymbol">
        ...
        <xsl:element name="csymbol">
            <xsl:choose>
                <xsl:when test="count(following-sibling::*) = 1">
                    <xsl:attribute name="cd">
                        <xsl:value-of select="$symbols//element/op[string()='unary_minus']/../cd"/>
                    </xsl:attribute>
                    <xsl:value-of select="$symbols//element/op[string()='unary_minus']/../name"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:attribute name="cd">
                        <xsl:value-of select="$symbols//element/op[string()='minus']/../cd"/>
                    </xsl:attribute>
        ...
        <xsl:element name="apply">
            <xsl:choose>
                <xsl:when test="matrix or count(./*) > 3">
                    <xsl:element name="csymbol"><xsl:attribute name="cd">linalg1</xsl:attribute>matrix_selector
                    </xsl:element>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:element name="csymbol"><xsl:attribute name="cd">linagl1</xsl:attribute>vector_selector
                    </xsl:element>
                </xsl:otherwise>
            </xsl:choose>
        ...
        <xsl:element name="semantics">
            ...
            <xsl:for-each select="@*[contains(name(),':')]">
                <xsl:element name="annotation">
                    ...
                    <xsl:attribute name="encoding">MathML-Content</xsl:attribute>
                    <xsl:element name="apply">
                        <xsl:element name="csymbol"><xsl:attribute name="cd">mathmlattr</xsl:attribute>foreign_attribute
                        </xsl:element>
                        <xsl:element name="cs">
    ...
    <xsl:function name="dsi:is-nary-reln" as="xs:string">
        ...
        <xsl:choose>
		    ...
		    or $elt/name()='lt' or $elt/name()='leq'">true
            </xsl:when>
    ...
    <xsl:function name="dsi:is-nary-set" as="xs:string">
        <xsl:param name="elt"/>
        <xsl:choose>
            <xsl:when test="$elt/name()='union' or $elt/name()='intersection' or $elt/name()='cartesianproduct'">true
            </xsl:when>
            <xsl:otherwise>false</xsl:otherwise>
        </xsl:choose>
    </xsl:function>
    ...
    <xsl:function name="dsi:name-from-mmlp" as="xs:string">
        ...
        <xsl:variable name="ret">
            <xsl:value-of select="translate(normalize-space($elt),' ','')"/><xsl:text>-</xsl:text>
